...
class S4Dataset(Dataset):
    def __init__(self, config, dataset_type):
        ...
        fold_data = pd.read_csv(os.path.join(config.data.folds_path, f'{config.data.fold}.csv'))
        self.data = fold_data.loc[:, f'{dataset_type}'].dropna()
        self.label = fold_data.loc[:, f'{dataset_type}_label'].dropna()
